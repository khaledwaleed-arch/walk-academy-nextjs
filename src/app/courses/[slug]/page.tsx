    ...
    <div className="min-h-screen bg-[#F5F5F5]">
      ...
      <header className="bg-gradient-to-br from-[#0D3B5C] to-[#1a5a8a] text-white py-16 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-xl text-white/70 mb-2">{ar.title}</p>
          <p className="text-white/60 text-lg mt-4 max-w-2xl">{en.tagline}</p>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-12">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
            ...
            <p className="text-gray-600 leading-relaxed">{en.description}</p>
          </div>
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100" dir="rtl">
            ...
            <p className="text-gray-600 leading-relaxed">{ar.description}</p>
          </div>
        </div>
              ...
              <details key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden group" open={i === 0}>
                <summary className="flex items-center justify-between px-6 py-4 cursor-pointer list-none hover:bg-gray-50 transition-colors">
                  <span className="font-semibold text-[#0D3B5C] flex items-center gap-3">
                    <span className="w-7 h-7 rounded-full bg-[#0D3B5C] text-white text-xs flex items-center justify-center font-bold flex-shrink-0">
                      {i + 1}
                    </span>
                    {mod.title}
                  </span>
                ...
                <div className="px-6 pb-5 border-t border-gray-50">
                  <ul className="mt-4 space-y-2">
                    {mod.topics.map((topic, j) => (
                      <li key={j} className="flex items-center gap-3 text-gray-600 text-sm">
                        <i className="fas fa-play-circle text-[#F58220] text-xs flex-shrink-0" />
                        {topic}
                      </li>
                    ))}
                  </ul>
                  ...
                  {ar.modules[i] && (
                    <ul className="mt-4 space-y-2 border-t border-gray-50 pt-4" dir="rtl">
                      {ar.modules[i].topics.map((topic, j) => (
                        <li key={j} className="flex items-center gap-3 text-gray-400 text-xs">
                          <i className="fas fa-play-circle text-[#0D3B5C]/30 text-xs flex-shrink-0" />
                          {topic}
                        </li>
                      ))}
                    </ul>
          ...
          <ul className="space-y-3">
            {en.audience.map((a, i) => (
              <li key={i} className="flex items-start gap-3 text-gray-700 text-sm">
                <i className="fas fa-user-check text-[#F58220] mt-0.5 flex-shrink-0" />
                {a}
              </li>
            ))}
          </ul>
            ...
            {courses
              ...
              .map((c) => (
                ...
                >
                  ...
                  <p className="text-gray-400 text-xs mt-1">{c.en.duration} · {c.price}</p>
                </Link>
              ))}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-[#0D3B5C] to-[#1a5a8a] rounded-3xl p-10 text-center text-white">
          ...
          >
            <i className="fas fa-graduation-cap" /> Enroll in {en.title}
          </Link>
        </div>
      </main>
    </div>
