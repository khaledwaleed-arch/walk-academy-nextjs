...
interface Stats {
  ...
  today_registrations: number; today_contacts: number;
}
...
function Dashboard({ token, onLogout }: { token: string; onLogout: () => void }) {
  const [tab, setTab] = useState<"regs" | "contacts" | "subs">("regs");
  ...
  const fetchAll = useCallback(async () => {
    ...
    setStats(s); setRegs(r); setContacts(c);
  }, [token]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => { fetchAll(); }, [fetchAll]);

  async function updateStatus(type: "registrations" | "contacts", id: number, status: string) {
        ...
        <div className="bg-white rounded-2xl p-4 mb-6 shadow-sm border border-gray-100 flex flex-wrap items-center gap-3">
          ...
          <div className="flex rounded-xl overflow-hidden border border-gray-200">
            {[
              ...
              <button key={t.key} onClick={() => { setTab(t.key as "regs" | "contacts"); setSearch(""); setStatusFilter("all"); }}
