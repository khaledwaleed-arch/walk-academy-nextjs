...
export default function ConsultationsPage() {
  ...
  const load = useCallback(async () => {
    const res = await fetch("/api/admin/consultations", { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div dir={dir}>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">{t("consultationsTitle")}</h1>
      ...
      <div className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-start px-4 py-3 font-medium text-gray-600 hidden md:table-cell">{t("companyCol")}</th>
                  <th className="text-start px-4 py-3 font-medium text-gray-600">{t("serviceCol")}</th>
                  <th className="text-start px-4 py-3 font-medium text-gray-600 hidden lg:table-cell">{t("appointmentCol")}</th>
                  <th className="text-start px-4 py-3 font-medium text-gray-600 hidden lg:table-cell">{t("messageCol")}</th>
                  <th className="text-start px-4 py-3 font-medium text-gray-600">{t("statusCol")}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="text-center py-12 text-gray-400">{t("noConsultations")}</td></tr>
                ) : filtered.map((r, i) => (
        ...
        <div className="px-4 py-2 text-xs text-gray-400 border-t border-gray-100">
          {t("showing")} {filtered.length} {t("ofWord")} {rows.length} {t("consultWord")}
        </div>
      </div>
    </div>
  );
