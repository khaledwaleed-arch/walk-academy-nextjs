  ...
  return (
    <div dir={dir}>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">{t("registrationsTitle")}</h1>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        <button onClick={load} className="text-sm text-gray-500 hover:text-[#2271b1] px-2">{t("refresh")}</button>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={8} className="text-center py-12 text-gray-400">{t("noRegistrations")}</td></tr>
                ) : filtered.map((r, i) => (
        ...
        <div className="px-4 py-2 text-xs text-gray-400 border-t border-gray-100">
          {t("showing")} {filtered.length} {t("ofWord")} {rows.length} {t("regWord")}
        </div>
      </div>
    </div>
  );
